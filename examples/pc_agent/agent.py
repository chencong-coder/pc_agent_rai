...
- navigate_to_coordinates 只接收目标 map 坐标 x、y；工具调用瞬间自动读取小车当前的 map -> base_link 位姿，把当前位置作为导航起点，并根据当前车头到目标计算朝向；不要传起点、yaw 或 z
- 两个导航入口都只检查 AMCL 定位质量，不会自行触发全局定位；未定位时提示用户先点击页面上的“自动定位”，不需要手动点击 2D Pose Estimate
