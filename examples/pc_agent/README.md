...
导航工具只需要 `x、y`。点击页面的“自动定位”后，若 Agent 已收到（包括点击前收到的）最新 `/initialpose`，AMCL 直接从该初始位姿确认，不控制小车旋转；如果没有收到初始位姿，则自动调用 AMCL 全局重定位并让小车原地旋转搜索。AMCL 必须连续 3 帧收敛才会生成有效位姿；工具使用最新的有效位姿计算朝向 `yaw`，并将二维目标的 `z` 固定为 `0`。定位失败、超时、取消或数据过期后，导航会拒绝启动，直到再次定位成功。
