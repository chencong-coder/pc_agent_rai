...
导航工具只需要 `x、y`。推荐先在 RViz 发布正确的 `2D Pose Estimate`，再点击页面的“自动定位”。Agent 会持续保存收到的最新 `/initialpose`，点击按钮时让 AMCL 从该初始位姿收敛；只有 Agent 从未收到过初始位姿时，才回退到 AMCL 全局定位。AMCL 必须连续 3 帧收敛才会生成有效位姿；工具使用最新的有效位姿计算朝向 `yaw`，并将二维目标的 `z` 固定为 `0`。定位失败、超时、取消或数据过期后，导航会拒绝启动，直到再次定位成功。
