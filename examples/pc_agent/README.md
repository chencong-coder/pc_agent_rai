...
导航工具只需要 `x、y`。点击页面的“自动定位”后，若 Agent 已收到（包括点击前收到的）最新 `/initialpose`，立即将它作为已确认的定位起点，不控制小车旋转；后续 `/amcl_pose` 用于实时更新坐标。如果没有收到初始位姿，则自动调用 AMCL 全局重定位并让小车原地旋转搜索，只有该分支要求连续 3 帧收敛。工具使用最新的有效位姿计算朝向 `yaw`，并将二维目标的 `z` 固定为 `0`。定位失败、超时、取消或数据过期后，导航会拒绝启动，直到再次定位成功。
