...
导航工具只需要 `x、y`。Agent 持续保存收到的最新 `/initialpose`；点击页面的“自动定位”后，AMCL 直接从该初始位姿确认，过程中不会调用全局重定位，也不会控制小车旋转。没有收到 `/initialpose` 时，定位会提示先设置初始位姿。AMCL 必须连续 3 帧收敛才会生成有效位姿；工具使用最新的有效位姿计算朝向 `yaw`，并将二维目标的 `z` 固定为 `0`。定位失败、超时、取消或数据过期后，导航会拒绝启动，直到再次定位成功。
