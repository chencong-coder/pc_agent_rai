...
Streamlit 页面顶部每秒刷新小车的 `map` 坐标 `x/y`、朝向 `yaw`（弧度）和
本地更新时间，读取的是 `map -> base_link` TF，不需要调用大模型。时间默认按
`Asia/Shanghai` 显示；如部署地区不同，可通过 `PC_AGENT_TIMEZONE` 指定时区。
尚未定位时显示等待状态；TF 时间与 ROS 当前时间相差超过 3 秒时标记数据过期，
保留最后已知位置及其原始时间。此功能只显示位置，不会自动设置初始位姿。
